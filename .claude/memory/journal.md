...
Session côté vidéo `videos/pawzzle-village-lore` (HyperFrames), sans rapport avec le code du jeu. Baptiste a signalé entendre toujours un son au moment du glow ambré + croix X de la "zone interdite" dans `compositions/scene4.html`, alors qu'un agent précédent avait retiré ce cue (`paw_incorrect.mp3`) et validé le retrait par grep + snapshot visuel + `npx hyperframes check` — jugé insuffisant (déjà vu 2 fois sur ce projet pour watermark/désynchro audio). Recherche exhaustive menée : aucun second cue caché, aucune scène dupliquée, un seul process de serveur de dev (tué et relancé à froid), un seul projet enregistré côté CLI. `ffmpeg`/`ffprobe` absents de la machine, bootstrappés temporairement via npm (`ffmpeg-static`/`ffprobe-static`) pour analyser objectivement un rendu réel : aucun cue distinct détecté dans la fenêtre visée, seulement la houle continue de la musique d'ambiance — méthode validée par 2 contrôles positifs (vrais SFX `paw_correct` détectés nettement en un pic net de ~9dB). Conclusion : pas de bug de code, cause probable côté utilisateur (onglet périmé / coïncidence perceptuelle) ([ZBLK-20260727233635-6](archive/blockers/ZBLK-20260727233635-6.md), [LRN-20260727233635-2](learnings/LRN-20260727233635-2.md)).

Sur validation de Baptiste ("ok. lance le rendu"), rendu final lancé (`--quality high`). Premier essai échoué : `assets/paw_correct.mp3`/`paw_incorrect.mp3` disparus du disque en cours de session (corrélé à un rendu précédent tué en plein calcul, projet avec `media.autoProxy: true`) — fichiers restaurés depuis `pawzzle/public/sounds/` (source d'origine), rendu final relancé avec succès (`renders/pawzzle-village-lore_2026-07-27_23-21-12.mp4`, 31.6s, vidéo+audio confirmés par `ffprobe`) ([ZBLK-20260727233635-7](archive/blockers/ZBLK-20260727233635-7.md), [LRN-20260727233635-5](learnings/LRN-20260727233635-5.md)). Baptiste a ensuite demandé la suppression des deux fichiers en affirmant qu'aucun n'était utilisé — grep a confirmé `paw_incorrect.mp3` mort (supprimé) mais infirmé le cas de `paw_correct.mp3` (6 références actives, conservé) ([BDR-20260727233635-1](decisions/BDR-20260727233635-1.md)). Deux frictions Windows/CLI annexes rencontrées et documentées : PATH Bash invisible pour un sous-process npx ([LRN-20260727233635-3](learnings/LRN-20260727233635-3.md)) et échec de rendu direct d'une sous-composition `<template>` via `-c` ([LRN-20260727233635-4](learnings/LRN-20260727233635-4.md)).

Sur choix explicite de Baptiste, toutes les entrées de cette session gardées 🏠 locales (pas de promotion globale).

**Entrées clés :**

- [ZBLK-20260727233635-6](archive/blockers/ZBLK-20260727233635-6.md) — cue audio fantôme, code confirmé propre par rendu réel (résolu)
- [ZBLK-20260727233635-7](archive/blockers/ZBLK-20260727233635-7.md) — assets SFX disparus en pleine session, restaurés (résolu)
- [BDR-20260727233635-1](decisions/BDR-20260727233635-1.md) — `paw_incorrect.mp3` supprimé, `paw_correct.mp3` conservé
- [LRN-20260727233635-2](learnings/LRN-20260727233635-2.md) — prouver l'absence d'un cue audio par rendu réel + contrôle positif

## 2026-07-28

Session dédiée au contenu social Instagram pour Pawzzle. Recherche initiale de skills spécialisés (carrousel + mini-vidéo, proposer+juger) sur le web puis skills.sh via agent-browser — meilleur candidat identifié (Blotato content-coach/post-grader) mais écarté par prudence (2 installs seulement, skill vu pour la première fois 12 jours avant). Baptiste a choisi d'avancer sans skill, en s'appuyant sur `/rodin` comme boucle de critique manuelle.

Trois rounds de propositions de publications (carrousels/images, DA "Atelier Feutrine" pastel/zen), chacun stress-testé par `/rodin` : Round 1 a fusionné 3 idées redondantes (règles/anatomie/astuces) en un seul carrousel fonctionnel et écarté les doublons vidéo/niche ; Round 2 a corrigé des hooks slide-1 hérités à tort de la logique vidéo (zéro-texte ne marche pas pareil sur un carrousel statique) ; Round 3 a détecté un risque légal réel (UI d'un concurrent affichée dans un visuel comparatif) et validé qu'une invitation à deviner en commentaire n'est pas une micro-compétition contrairement au speedrun déjà écarté. 7 formats retenus au total avec plan de publication ordonné (voir [BDR-20260728232006-1](decisions/BDR-20260728232006-1.md)), tracking complet dans `docs/social-content-publication-tracking.md`.

Production du premier carrousel ("Comment on s'installe dans le village") sans skill externe, sur demande explicite de Baptiste : script Node réutilisant les vrais tokens du jeu (`REGION_COLORS`, squircle, palette) pour générer 6 slides HTML, puis `agent-browser` pour le screenshot 1080×1350 ([BDR-20260728232006-2](decisions/BDR-20260728232006-2.md)). Deux frictions rencontrées et corrigées en cours de route : un chemin `file://` mal résolu par Git Bash sur Windows ([BLK-20260728232006-6](blockers/BLK-20260728232006-6.md)) et un logo caché derrière la grille par un z-index implicite, plus un débordement vertical du CTA une fois corrigé ([BLK-20260728232006-7](blockers/BLK-20260728232006-7.md)). Un mini-viewer HTML swipeable a ensuite été construit et publié en Artifact pour prévisualiser le résultat (fonts Fredoka/IBM Plex embarquées en base64, palette Atelier Feutrine reprise à l'identique).

**Entrées clés :**

- [BDR-20260728232006-1](decisions/BDR-20260728232006-1.md) — stratégie contenu Instagram, 7 formats retenus via /rodin
- [BDR-20260728232006-2](decisions/BDR-20260728232006-2.md) — pipeline de production carrousel sans skill
- [BLK-20260728232006-7](blockers/BLK-20260728232006-7.md) — logo caché par z-index implicite + overflow CTA
